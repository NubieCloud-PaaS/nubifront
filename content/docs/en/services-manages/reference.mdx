...
## Managed applications: what they share

Keycloak, n8n, Metabase, Hasura, DocuSeal and Open WebUI follow the same model: an application container, a public URL, and **a dedicated database created for them on a managed PostgreSQL** you choose. Two things apply to all of them.

### The dedicated database credentials

At the end of the deployment, the console shows **once** — together with the administrator account or the temporary access, depending on the application — the credentials of the database created on your PostgreSQL: host, port, database name, user, password and a ready-to-paste connection URL.

They are yours: plug in a BI tool, export the data, or migrate it elsewhere. The host is the internal address of your PostgreSQL, reachable from your applications in this environment, not from the Internet.

> ⚠️ **Save this password before closing the window.** Like the administrator's, it is not shown again: the database stays listed among the additional databases of your PostgreSQL, without its password. If you lose it, contact support.

### Extra environment variables

From **Edit**, you can add environment variables to the application container, on top of those Nubiecloud sets (database, administrator account, public address). This is how you tune the options the application reads from its environment — for Open WebUI, for instance, connecting a Redis, setting persistent parameters or disabling sign-up.

Three rules:

- The available names are those of **the application's documentation**, not Nubiecloud's.
- A variable with the same name as one set by the platform **overrides it**. You can therefore break your instance; removing it is enough to go back.
- **Never put a secret there.** These values are stored in plain text. An API key or a password goes into the **secrets**, right below.

Each save replaces the whole list: a variable removed from the screen disappears from the instance at the next redeploy.

### Secrets

Same screen, **Secrets** section: a model provider API key for Open WebUI, an SMTP password for n8n or Keycloak. They are injected into the container as environment variables, encrypted at rest, and changing one restarts the application.

A saved value is **never shown again**. On the next edit, the row appears with its name and an empty field: leave it empty to keep the value, type one to replace it, remove the row to delete the secret.

A variable set by Nubiecloud keeps precedence over a secret of the same name: you cannot replace the database or the administrator account this way.

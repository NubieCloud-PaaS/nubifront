...
The replica count determines fault tolerance, and it is not something you change lightly after the fact:

| Replicas | Replication factor | Tolerance | Use |
|:--------:|:------------------:|-----------|-----|
| **1** | 1 | none — losing the node loses the messages | development, proof of concept |
| **3** | 3 (`min.insync.replicas` = 2) | survives the loss of one node | production |

> ⚠️ **A single-replica Kafka is not a production Kafka.** No copy exists elsewhere: if the node goes down, unconsumed messages are lost. Choose 3 replicas as soon as the messages matter.
...
The **data volume survives** deletion of the deployment. This is deliberate — it protects against accidental deletion — but it means the disk space stays allocated. Contact support if you want it released.

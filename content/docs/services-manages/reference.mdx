...
### Réplicas et tolérance de panne

Le nombre de réplicas détermine la résistance à la panne, et il ne se change pas d'un claquement de doigts après coup :

| Réplicas | Facteur de réplication | Tolérance | Usage |
|:--------:|:----------------------:|-----------|-------|
| **1** | 1 | aucune — la perte du nœud perd les messages | développement, maquette |
| **3** | 3 (`min.insync.replicas` = 2) | supporte la perte d'un nœud | production |

> ⚠️ **Un Kafka à 1 réplica n'est pas un Kafka de production.** Aucune copie n'existe ailleurs : si le nœud tombe, les messages non consommés sont perdus. Choisissez 3 réplicas dès que les messages comptent.

...
## Applications managées : ce qui leur est commun

Keycloak, n8n, Metabase, Hasura, DocuSeal et Open WebUI partagent le même modèle : un conteneur applicatif, une URL publique, et **une base dédiée créée pour elles sur un PostgreSQL managé** que vous choisissez. Deux choses valent pour toutes.

### Les identifiants de la base dédiée

À la fin du déploiement, la console affiche **une seule fois** — avec le compte administrateur ou l'accès temporaire, selon l'application — les identifiants de la base créée sur votre PostgreSQL : hôte, port, nom de la base, utilisateur, mot de passe et URL de connexion prête à coller.

Ils sont à vous : branchez un outil de BI, exportez les données, ou migrez-les ailleurs. L'hôte est l'adresse interne de votre PostgreSQL, joignable depuis vos applications de cet environnement, pas depuis Internet.

> ⚠️ **Notez ce mot de passe avant de fermer la fenêtre.** Comme celui de l'administrateur, il n'est pas réaffiché : la base reste visible dans la liste des bases additionnelles de votre PostgreSQL, sans son mot de passe. Si vous le perdez, contactez le support.

### Variables d'environnement supplémentaires

Depuis **Modifier**, vous pouvez ajouter des variables d'environnement au conteneur de l'application, en plus de celles que Nubiecloud pose (base de données, compte administrateur, adresse publique). C'est ainsi que se règlent les options que l'application lit dans son environnement — par exemple, pour Open WebUI, brancher un Redis, fixer les paramètres persistants ou désactiver l'inscription.

Trois règles :

- Les noms disponibles sont ceux de **la documentation de l'application**, pas de Nubiecloud.
- Une variable qui porte le même nom qu'une variable posée par la plateforme **la remplace**. Vous pouvez donc casser votre instance ; la retirer suffit à revenir en arrière.
- **N'y mettez aucun secret.** Ces valeurs sont stockées en clair. Une clé d'API ou un mot de passe va dans les **secrets**, juste en dessous.

Chaque enregistrement remplace la liste complète : une variable retirée de l'écran disparaît de l'instance au redéploiement suivant.

### Secrets

Même écran, section **Secrets** : une clé d'API de fournisseur de modèles pour Open WebUI, un mot de passe SMTP pour n8n ou Keycloak. Ils sont injectés dans le conteneur comme variables d'environnement, chiffrés au repos, et changer l'un d'eux redémarre l'application.

Une valeur enregistrée n'est **jamais réaffichée**. À la modification suivante, la ligne apparaît avec son nom et un champ vide : laissez-le vide pour conserver la valeur, saisissez-en une pour la remplacer, supprimez la ligne pour retirer le secret.

Une variable posée par Nubiecloud garde la main sur un secret de même nom : vous ne pouvez pas remplacer la base ou le compte administrateur par ce biais.

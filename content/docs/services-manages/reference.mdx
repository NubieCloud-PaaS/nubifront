...
API bancaire ouverte : comptes, transactions, consentements et rôles, exposés selon les standards Open Banking et PSD2. C'est une **API**, pas une interface — elle s'adresse à vos développeurs.

| Caractéristique | Valeur |
|-----------------|--------|
| Version | épinglée par **commit** (`a98ba98`) — voir ci-dessous |
| CPU / RAM par défaut | 1 cœur / 2 Go |
| Stockage | **aucun volume** — les 149 tables vivent dans un PostgreSQL managé |
| Accès | URL publique, en HTTPS |
| Réplicas | 1 (instance unique) |
| Sauvegardes | via celles de son PostgreSQL hôte |

> ⚠️ **Le compte d'administration n'existe pas au démarrage, et il ne se crée pas tout seul.** Lisez la section suivante avant de déployer : c'est la seule chose qui distingue une instance utilisable d'une instance vide.

### Le compte administrateur

Open Bank Project n'a **aucun** administrateur à sa création, et **s'inscrire soi-même sur son adresse publique n'en fait pas un**. C'est la particularité à comprendre :

- l'inscription est **ouverte sans authentification** — c'est le modèle d'Open Bank Project, pensé pour un bac à sable où des développeurs s'enregistrent ;
- mais un compte ainsi créé n'a **aucun droit** : ni banque, ni compte, ni transaction. L'accès est porté par des **entitlements** (rôles), qu'un administrateur seul peut accorder.

Le statut d'administrateur vient uniquement de la configuration du déploiement. Il se pose **depuis la console** :

1. ouvrez la fiche du déploiement ;
2. cliquez sur **« Créer l'administrateur »** ;
3. **notez le mot de passe affiché** — il n'est montré qu'une fois et n'est stocké nulle part ;
4. l'application redémarre pour prendre en compte ces droits (environ deux minutes).

Le compte créé porte **votre propre adresse e-mail**. Une autre personne de votre organisation peut ensuite s'ajouter à son tour par le même bouton : cela ne retire ses droits à personne.

### Pas d'accès temporaire, et pourquoi

Contrairement à n8n, Metabase et DocuSeal, Open Bank Project **n'a pas d'authentification provisoire** posée devant son adresse pendant l'installation. Ce n'est pas un oubli : chez ces applications, le premier visiteur devient propriétaire de l'instance, ce qui justifie de fermer la porte le temps que vous arriviez. Ici, le premier visiteur ne devient rien du tout — la porte protégerait donc contre un risque inexistant, tout en vous empêchant vous-même d'utiliser l'API.

### Le premier démarrage

Comptez **environ deux minutes** : c'est une application Java, qui crée ses 149 tables au premier lancement. Une page d'erreur pendant ce laps de temps est normale.

### La page d'accueil et son annuaire

À la racine de votre instance, Open Bank Project affiche un **annuaire des applications** de son écosystème — API Explorer, Portal, Opey, MCP… La plupart y apparaissent comme *« Public URL is not set »*, et **c'est normal** : Nubiecloud déploie l'API, pas ces applications satellites. Seule l'entrée **OBP API** porte une adresse, la vôtre.

Les liens utiles sont plus bas sur cette même page : **API Documentation** (la liste complète des endpoints de votre instance) et **Banks**.
...
### Une version épinglée par commit

Open Bank Project ne publie **aucune version** au sens habituel : ses derniers tags datent de 2023 et son artefact vivant est reconstruit en continu. Nubiecloud épingle donc un **commit** précis, vérifié, plutôt qu'une étiquette mouvante — c'est ce qui garantit que votre instance ne change pas sous vos pieds. L'API se déclare elle-même `BLEEDING_EDGE`.

### À la suppression

Supprimer le déploiement **détruit sa base de données** : comptes, transactions, consentements, entitlements. La console vous l'annonce dans la fenêtre de confirmation.

### Limites actuelles

Instance unique. Connecteur `mapped` : Open Bank Project range les données dans **sa propre base**, il n'est pas branché sur un système bancaire existant. Licence AGPL-3.0.
